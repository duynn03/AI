...
for i in range(epochs):
    with tf.GradientTape() as tape:
        ...
        loss = tf.losses.MeanSquaredError(y_train, y_pred)  # <- Compute error of prediction with ground truth
    gradients = tape.gradient(loss, [w, b])  # <- Calculate gradients of loss w.r.t weights (m, b)

    # update w & b parameters
    optimizer.apply_gradients(zip(gradients, [w, b])) # <- Update weights (one step gradient descent)
